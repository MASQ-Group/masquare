 ...
 *                    can check the amount and must stay silent on the rest.
 *
 * Reads only.
 */
const fs = require('fs');
...
(async () => {
  ...
  for (const key of ['AmazonAmerica', 'AmazonAUS', 'AmazonJP', 'AmazonSA', 'AmazonAE', 'AmazonSG']) {
    ...
    for (const [ref, f] of entries) {
      ...
      theirTotal += theirs; ourTotal += vat;
      if (t.vatCollectedByChannel) flagged += 1;
      regimes[t.taxType ?? 'null'] = (regimes[t.taxType ?? 'null'] ?? 0) + 1;
      if (near(vat, theirs, Math.max(0.05, Math.abs(theirs) * 0.02))) agree += 1;
      else { wrong += 1; if (ex.length < 5) ex.push(`${ref} -> ${f.dest}  amazon ${m(theirs)}  ours ${m(vat)}  (price ${m(f.price)})`); }
    }
    console.log(`\n══════ ${key} — ${entries.length} order(s) ══════`);
    console.log(`  not in our database        ${missing}`);
    console.log(`  tax agrees                 ${agree}`);
    console.log(`  tax differs                ${wrong}`);
    console.log(`  flagged as channel-collected ${flagged}`);
    console.log(`  tax: amazon ${m(theirTotal)}   ours ${m(ourTotal)}   difference ${m(ourTotal - theirTotal)}`);
