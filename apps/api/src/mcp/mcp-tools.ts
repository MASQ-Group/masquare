...
export const INSTRUCTIONS = [
  ...
  '1. get_product_for_gather with channel "all" - it first CHOOSES any category nobody has chosen (see',
  '   categoriesChosen in the reply), because a category decides which fields exist. Then one list of',
  '   what EVERY channel is waiting for, folded so',
  ...
  '   is a measurement: report the number with ONE OF THOSE UNITS, exactly as the page prints it. A detail',
  '   whose "units" is null takes plain text — a colour, a material, a warranty — so write what the page says',
  '   and do not invent a unit for it. It also lists pagesAlreadyAccepted',
      ...
      messages: [{
        ...
        content: {
          ...
          text: [
            ...
            '',
            'For each product:',
            '1. get_product_for_gather with channel "all". It first CHOOSES any category nobody has chosen,',
            '   because a category decides which fields exist - see categoriesChosen in the reply. Then it',
            '   gives ONE list of what every channel is waiting for, folded so a fact several marketplaces ask',
            '   for appears once and names them all, plus alreadyKnown - what the platform already holds. Do',
            '   not go looking for anything in alreadyKnown again unless a page you are reading anyway states it.',
            '   If a channel still has no category, the marketplace and our own history disagreed about where',
            '   this product goes. Tell the user both answers from categoriesChosen and carry on with the rest:',
            '   that is a judgement for somebody who knows the product, not something to work around.',
