...
// Port and API target come from the env file the dev script loads (both `dev` and `dev:demo`
// go through dotenv-cli), so the same code serves the live workspace (.env → :5173 → :3200)
// and the demo one (.env.demo → :5273 → :3100).
//
// The fallback below is a last resort, not a default anyone should hit: when `dev` did not
// load .env, the proxy silently pointed at :3000 while the API listened elsewhere, and every
// request 404'd through a proxy that looked correctly configured.
