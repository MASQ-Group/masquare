...
const FIELDS = [
  ...
  { key: 'feeChargedInNativeCurrency', label: 'Fee In Native Currency' },
...
export function SalesChannelImportModal({ channels, resolveCountryId, onClose, onDone }: Props) {
  ...
  const bodyOf = (r: Record<string, string>) => ({
    ...
    feeChargedInNativeCurrency: !/^(n|no|false|0)$/i.test((r.feeChargedInNativeCurrency ?? '').trim()),
    feeCurrency: r.feeCurrency || undefined,
