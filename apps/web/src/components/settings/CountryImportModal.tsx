...
import { countriesApi, type Country, type ShippingService } from '../../lib/api';
...
type RowResult = { index: number; iso: string; name: string; status: 'new' | 'exists'; existingId: string | null };

const FIELDS = [
  { key: 'name', label: 'Country Name' },
  { key: 'isoCode', label: 'ISO Code' },
  { key: 'continent', label: 'Continent' },
  { key: 'euVatZone', label: 'EU VAT Zone' },
  { key: 'vatRate', label: 'VAT Rate' },
  { key: 'defaultShippingService', label: 'Default Shipping Service' },
];
...
export function CountryImportModal({ countries, services, onClose, onDone }: Props) {
  ...
  const resolveServiceId = (token?: string) => {
    const t = (token ?? '').trim();
    if (!t) return null;
    const s = services.find((x) => x.name.toLowerCase() === t.toLowerCase() || (x.alias ?? '').toLowerCase() === t.toLowerCase());
    return s?.id ?? null;
  };
  ...
  const onFile = async (file: File) => {
    ...
    const mapped = raw
      ...
      .filter((r) => (r.isoCode ?? '').trim() || (r.name ?? '').trim());
    setRows(mapped);
  };

  const runReview = () => {
    if (rows.length === 0) { toast.error('Upload a file with at least one country'); return; }
    const byIso = new Map(countries.map((c) => [c.isoCode.toUpperCase(), c]));
    const res: RowResult[] = rows.map((r, i) => {
      const iso = (r.isoCode ?? '').trim().toUpperCase();
      const existing = byIso.get(iso);
      return { index: i, iso, name: (r.name ?? '').trim(), status: existing ? 'exists' : 'new', existingId: existing?.id ?? null };
    });
    setResults(res);
    setActions(res.map((r) => (r.status === 'exists' ? { action: 'edit', existingId: r.existingId ?? undefined } : { action: 'add' })));
    setStep('review');
  };

  const bodyOf = (r: Record<string, string>) => ({
    name: (r.name ?? '').trim(),
    isoCode: (r.isoCode ?? '').trim().toUpperCase(),
    continent: (r.continent ?? '').trim(),
    euVatZone: truthy(r.euVatZone),
    vatRate: Number((r.vatRate ?? '0').replace('%', '').trim()) || 0,
    defaultShippingServiceId: resolveServiceId(r.defaultShippingService),
  });
  ...
  const commit = async () => {
    ...
    try {
      for (let i = 0; i < rows.length; i++) {
        ...
        const body = bodyOf(rows[i]);
        if (!body.name || !body.isoCode || !body.continent) { errors.push(`${body.isoCode || body.name || `row ${i + 1}`}: Name, ISO Code and Continent are required`); continue; }
        try {
          if (a.action === 'edit' && a.existingId) { await countriesApi.update(a.existingId, body); updated++; }
          else { await countriesApi.create(body); created++; }
        } catch (e: any) {
          errors.push(`${body.isoCode}: ${e?.response?.data?.message ?? 'failed'}`);
        }
      }
  ...
  return (
    <ModalShell open title="Import Countries" subtitle={fileName || undefined} primaryLabel={primary.label} onPrimary={primary.onClick} primaryDisabled={primary.disabled} busy={busy} onClose={onClose}>
      {step === 'setup' && (
        <div className="flex flex-col gap-5">
          <div>
            <label className="label">What is this file for?</label>
            <div className="flex gap-2">
              {([['add', 'Add new countries'], ['edit', 'Edit existing countries']] as const).map(([val, lbl]) => (
            ...
            <p className="mt-1.5 text-[12px] text-n-400">
              {purpose === 'add'
                ? 'Rows whose ISO Code already exists will be flagged so you can choose update-or-drop.'
                : 'Rows whose ISO Code is not found will be flagged so you can choose add-as-new or drop. ISO Code is the unique identifier.'}
            </p>
          </div>
          ...
          <div className="max-h-72 overflow-auto rounded-lg border border-n-200">
            <table className="w-full border-collapse text-[12.5px]">
              <thead className="sticky top-0">
                <tr>{['#', 'ISO', 'Country', 'Status', 'Action'].map((h) => <th key={h} className="border-b border-n-200 bg-n-25 px-2 py-1.5 text-left font-semibold uppercase tracking-wide text-n-500">{h}</th>)}</tr>
              </thead>
              <tbody>
                {results.map((r, i) => {
                  ...
                  return (
                    <tr key={i} className={actions[i]?.action === 'skip' ? 'bg-warning-bg/30' : ''}>
                      ...
                      <td className="border-t border-n-100 px-2 py-1.5 font-mono text-n-800">{r.iso || '—'}</td>
                      <td className="border-t border-n-100 px-2 py-1.5 text-n-700">{r.name || '—'}</td>
